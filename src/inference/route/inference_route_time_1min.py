...
def predict(test_loader, model, device, target_scaler, num_samples=50, save_dir="results"):
    ...
    with torch.no_grad():
        for batch in test_loader:
            features, busrout_id, businfo_unit_id, time_gap, _ = [b.to(device) for b in batch]
...
if __name__ == "__main__":
    ...
    # 추론용 데이터 로드
    test_csv_min = '../../../dataset/inference/route/241007_모든노선_8.1~8.14_평일_특성추가/inf_filtered.csv'
    dtype_spec = {
        'DAY_TYPE': 'int8',
        'BUSROUTE_ID': 'str',
        'PEEK_ALLOC': 'int16',
    ...
    data_pd['LOG_ROUTE_LEN'] = np.log1p(data_pd['ROUTE_LEN'])   # log(1 + x) 변환
    ...
    model_path = ('../../../runs/train_results/route/20241007-080115/models'
                  '/travel_model_lr5.00e-02_dr3.00e-01_bs4096_hr20000_hu20000_re4_ue6.pth')
    model_dir = os.path.dirname(model_path)

    # travel_train.py 파일 경로 설정
    travel_train_py_dir = os.path.dirname(model_dir)
    travel_train_py_path = os.path.join(travel_train_py_dir, 'travel_route_train.py')
    ...
    # model_dir 안의 travel_route_train.py 파일에서 TravelTimeModel, TravelTimeDataset 불러오기
    module = load_module_from_path('travel_route_train', travel_train_py_path)
    ...
    data_loader = DataLoader(
        TravelTimeDataset(data_pd, scaler, target_scaler, route_hash_size, unit_hash_size, mode='train')
