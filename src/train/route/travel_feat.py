...
config = {
    "train_csv": '../../../dataset/train/route/241007_모든노선_8.1~8.14_평일_특성추가/소통_train_filtered_train.csv',
    "val_csv": '../../../dataset/train/route/241007_모든노선_8.1~8.14_평일_특성추가/소통_train_filtered_val.csv',
    "test_csv": '../../../dataset/train/route/241007_모든노선_8.1~8.14_평일_특성추가/소통_train_filtered_test.csv',
    "train_epochs": 300,
    "n_trial": 4,
    "trial_epochs": 5,
    "patience": 15,
...
class TravelTimeDataset(Dataset):
    def __init__(self, data, scaler=None, target_scaler=None,
                 route_hash_size=1500, unit_hash_size=1000, mode='train'):
        self.mode = mode  # 'train', 'inference'
        self.route_hash_size = route_hash_size
        self.unit_hash_size = unit_hash_size
        ...
        # self.speed = data['SPEED'].values

        # 해시로 변환된 구간 ID
        self.busrout_id = data['BUSROUTE_ID'].apply(lambda x: self.hash_function(x, self.route_hash_size)).values
        self.businfo_unit_id = data['BUSINFOUNIT_ID'].apply(lambda x: self.hash_function(x, self.unit_hash_size)).values

        # Scale new numerical features
        numerical_features = ['LEN', 'PEEK_ALLOC', 'NPEEK_ALLOC', 'BUSSTOP_CNT',
                              'INFOUNIT_SEQ', 'GPS_COORDX', 'GPS_COORDY', 'LOG_ROUTE_LEN']

        if scaler is None:
            self.scaler = StandardScaler()
            self.scaled_features = self.scaler.fit_transform(data[numerical_features].values)
        else:
            self.scaler = scaler
            self.scaled_features = self.scaler.transform(data[numerical_features].values)
        ...
        self.features = np.column_stack((
            ...
            self.scaled_features,      # New scaled numerical features
            # self.speed,           # 평균 속도 (km/h)
    ...
    def __getitem__(self, idx):
        ...
        distance_weight = 3 if self.len[idx] >= 100 else 1.0  # 100m 이상일 때 가중치 3배

        if self.mode == 'inference':   # 추론 시에는 time_gap을 반환하지 않음 (distance_weight도)
            return (
                torch.tensor(self.features[idx], dtype=torch.float),
                torch.tensor(self.busrout_id[idx], dtype=torch.long),
            ...
            return (
                ...
                torch.tensor(distance_weight, dtype=torch.float)
            )


class TravelTimeModel(nn.Module):
    def __init__(self, input_size, route_hash_size, unit_hash_size,
                 route_embedding_dim, unit_embedding_dim, dropout_rate):
        ...
        self.unit_embedding = nn.Embedding(num_embeddings=unit_hash_size, embedding_dim=unit_embedding_dim)

        # 더 많은 뉴런과 레이어 추가
        self.fc1 = nn.Linear(input_size + unit_embedding_dim + route_embedding_dim, 256)
    ...
    def forward(self, features, busrout_id, businfo_unit_id):
        route_emb = self.route_embedding(busrout_id)
        unit_emb = self.unit_embedding(businfo_unit_id).squeeze(1)
        inputs = torch.cat([features, route_emb, unit_emb], dim=1)
...
# 3. 데이터 로드 및 전처리
def load_data(config, batch_size=32, route_hash_size=None, unit_hash_size=None):
    def load_and_clean_data(csv_path):  # 빈 행, 값 0인 행, 띄어쓰기 제거 후 인덱스 리셋
        dtype_spec = {
            ...
            'GPS_COORDX': 'float32',
            'GPS_COORDY': 'float32',
            'COLLECT_DATE': 'str',  # Not used
            'DEP_TIME': 'str',
            'TIME_GAP': 'int32',
            'SPEED': 'int32'  # Not used
        }
        # usecols = ['DAY_TYPE', 'BUSINFOUNIT_ID', 'LEN', 'DEP_TIME', 'TIME_GAP']
        # usecols = ['DAY_TYPE', 'BUSINFOUNIT_ID', 'LEN', 'DEP_TIME', 'TIME_GAP', 'SPEED']
        # usecols = ['BUSINFOUNIT_ID', 'LEN', 'DEP_TIME', 'TIME_GAP', 'SPEED']
        # usecols = ['BUSINFOUNIT_ID', 'LEN', 'DEP_TIME', 'TIME_GAP']
        usecols = [
        ...
        # 데이터 로드 및 전처리 (결측치 제거 등)
        data = pd.read_csv(csv_path, skipinitialspace=True, usecols=usecols, dtype=dtype_spec).loc[
            lambda df: (df != 0).all(axis=1)
        ].dropna(how='all').reset_index(drop=True)

        # ROUTE_LEN에 로그 변환 적용 (만 단위라서)
        data['LOG_ROUTE_LEN'] = np.log1p(data['ROUTE_LEN'])  # log(1 + x) 변환
        return data
    ...
    numerical_features = ['PEEK_ALLOC', 'NPEEK_ALLOC', 'BUSSTOP_CNT', 'LEN',
                          'INFOUNIT_SEQ', 'GPS_COORDX', 'GPS_COORDY', 'LOG_ROUTE_LEN']

    # 모든 숫자형 특성에 대해 하나의 스케일러를 사용
    scaler = StandardScaler()
    scaler.fit(train_data[numerical_features].values)
    ...
    train_dataset = TravelTimeDataset(train_data, scaler, target_scaler, route_hash_size, unit_hash_size)
    val_dataset = TravelTimeDataset(val_data, scaler, target_scaler, route_hash_size, unit_hash_size)
    test_dataset = TravelTimeDataset(test_data, scaler, target_scaler, route_hash_size, unit_hash_size)
...
def objective(trial):
    ...
    route_hash_size = config['route_hash_size']    # Fixed or suggest as needed
    unit_hash_size = config['unit_hash_size']  # Fixed or suggest as needed
    route_embedding_dim = trial.suggest_int('route_embedding_dim', 32, 128)
    ...
    train_loader, val_loader, test_loader, target_scaler = load_data(
        config, batch_size, route_hash_size, unit_hash_size
    )
    ...
    # 모델 정의
    model = TravelTimeModel(input_size, route_hash_size, unit_hash_size,
                            route_embedding_dim, unit_embedding_dim, dropout_rate).to(device)
    ...
    # 개선된 옵티마이저 및 스케줄러 적용
    optimizer, scheduler = get_improved_optimizer_scheduler(model, config, lr, len(train_loader))
    ...
    for epoch in range(num_epochs):
        ...
        for i, batch in enumerate(tqdm(train_loader, desc=f"Epoch {epoch + 1}/{num_epochs}", unit="batch")):
            features, busrout_id, businfo_unit_id, time_gap, distance_weight = [b.to(device) for b in batch]
            optimizer.zero_grad()
            outputs = model(features, busrout_id, businfo_unit_id)
...
# 4-2. 학습 함수 정의
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, target_scaler, device, num_epochs=None, hyperparams=None):
...
def evaluate_model(model, val_loader, criterion, target_scaler, device):
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            # 손실 계산 (원래 스케일)
            # loss = weighted_mse_loss(outputs_original, time_gap_original, distance_weight.cpu().numpy())
            # total_loss += loss.item()
...
def evaluate_predictions(test_loader, model, device, target_scaler, num_samples=20):
    ...
    # 비교 테이블 저장
    table_path = os.path.join(paths['plots'], f'travel_time_comparison_table_{short_params_string(best_params)}.csv')
...
def get_improved_optimizer_scheduler(model, config, lr, steps_per_epoch):
    # 1. AdamW 사용 (가중치 감쇠가 더 잘 동작)
    optimizer = optim.AdamW(model.parameters(),
                           lr=config['lr'],
                           weight_decay=0.01)

    # 2. One Cycle Learning Rate 스케줄러 사용
    scheduler = optim.lr_scheduler.OneCycleLR(
        optimizer,
        max_lr=lr,
        epochs=config['train_epochs'],
        steps_per_epoch=steps_per_epoch,
        pct_start=0.3,  # 30% 지점까지 lr을 증가시키다가 감소
        div_factor=25,  # 초기 학습률 = max_lr/25
        final_div_factor=1e4  # 최종 학습률 = 초기 학습률/1e4
    )
...
def short_params_string(params):
    ...
    key_params = ['lr', 'dr', 'bs', 'hr', 'hu', 're', 'ue']
    values = [f"{params['lr']:.2e}", f"{params['dropout_rate']:.2e}", str(params['batch_size']),
              str(params['route_hash_size']), str(params['unit_hash_size']),
              str(params['route_embedding_dim']), str(params['unit_embedding_dim'])]
...
if __name__ == "__main__":
    # 결과 저장 폴더 기본 경로 생성
    base_dir = f'../../../runs/train_results/route/{time.strftime("%Y%m%d-%H%M%S")}'
    ...
    # 훈련 코드 파일을 결과 폴더에 복사
    shutil.copy2(current_code_path, os.path.join(base_dir, 'travel_route_train.py'))
    ...
    best_params = {
        'batch_size': 4096,
        'route_hash_size': config['route_hash_size'],
        'unit_hash_size': config['unit_hash_size'],
        'route_embedding_dim': config['route_embedding_dim'],
        'unit_embedding_dim': config['unit_embedding_dim'],
        'lr': 0.4,
        'dropout_rate': 0.3
    }
    ...
    unit_hash_size = best_params['unit_hash_size']
    ...
    # 학습된 모델 저장
    model_name = os.path.join(paths['models'], f'travel_model_{short_params_string(best_params)}.pth')
    ...
    # 필요한 컬럼들만 선택
    columns_to_save = ['DAY_TYPE', 'BUSROUTE_ID', 'BUSINFOUNIT_ID', 'LEN', 'DEP_TIME', 'TIME_GAP']
