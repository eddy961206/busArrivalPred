...
config = {
    "train_csv": '../../../dataset/train/travel/241004_노선모두_8.28~9.11_평일_속도X/소통_train_filtered_train.csv',
    "val_csv": '../../../dataset/train/travel/241004_노선모두_8.28~9.11_평일_속도X/소통_train_filtered_val.csv',
    "test_csv": '../../../dataset/train/travel/241004_노선모두_8.28~9.11_평일_속도X/소통_train_filtered_test.csv',
    "train_epochs": 300,
    "n_trial": 4,
    "trial_epochs": 5,
    "patience": 15,
...
class TravelTimeDataset(Dataset):
    def __init__(self, data, scaler=None, target_scaler=None, hash_size=1000, mode='train'):
        ...
        if scaler is None:
            ...
            self.len_scaled = self.scaler.fit_transform(self.len.reshape(-1, 1)).flatten()
        else:
            self.scaler = scaler
            self.len_scaled = self.scaler.transform(self.len.reshape(-1, 1)).flatten()
    ...
    def __getitem__(self, idx):
        ...
        distance_weight = 3.5 if self.len[idx] >= 500 else 1.0  # 1500m 이상일 때 가중치 2배
...
def load_data(config, batch_size=32, hash_size=1000):
    ...
    # Train 데이터셋에만 스케일러를 fit
    len_scaler = StandardScaler()
    len_scaler.fit(train_data['LEN'].values.reshape(-1, 1))
    ...
    train_dataset = TravelTimeDataset(train_data, len_scaler, target_scaler, hash_size)
    val_dataset = TravelTimeDataset(val_data, len_scaler, target_scaler, hash_size)
    test_dataset = TravelTimeDataset(test_data, len_scaler, target_scaler, hash_size)
    ...
    # 스케일러 저장 (추후 재사용을 위해)
    with open(os.path.join(paths['models'], 'len_scaler.pkl'), 'wb') as f:
        pickle.dump(len_scaler, f)
...
def objective(trial):
    ...
    train_loader, val_loader, test_loader, target_scaler = load_data(config, batch_size=batch_size, hash_size=hash_size)
...
if __name__ == "__main__":
    # 결과 저장 폴더 기본 경로 생성
    base_dir = f'../../../runs/train_results/travel/{time.strftime("%Y%m%d-%H%M%S")}'
    ...
    # 훈련 코드 파일을 결과 폴더에 복사
    shutil.copy2(current_code_path, os.path.join(base_dir, 'travel_train.py'))
