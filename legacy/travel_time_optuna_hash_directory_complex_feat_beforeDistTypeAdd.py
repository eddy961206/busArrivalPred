...
class TravelTimeDataset(Dataset):
    def __init__(self, data, hash_size=1000):
        ...
        # 구간 길이를 소요 시간으로 나눈 값 (km/h)
        self.len_per_time = self.speed_kmh
        ...
        self.features = np.column_stack((
            ...
            self.len_per_time,    # 구간 길이를 소요 시간으로 나눈 값 (km/h)
...
def objective(trial):
    ...
    for epoch in range(num_epochs):
        ...
        for i, batch in enumerate(tqdm(train_loader, desc=f"Epoch {epoch + 1}/{num_epochs}", unit="batch")):
            features, businfo_unit_id, time_gap = [b.to(device) for b in batch]
            optimizer.zero_grad()
