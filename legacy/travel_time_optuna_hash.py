...
class TravelTimeDataset(Dataset):
    def __init__(self, data, hash_size=1000):
        ...
        # 해시 인코딩을 사용하여 BUSINFOUNIT_ID를 인코딩
...
class TravelTimeModel(nn.Module):
    def __init__(self, input_size, hash_size, unit_embedding_dim):
...
# 3. 데이터 로드 및 전처리
def load_data(train_csv, val_csv, test_csv, batch_size=32, hash_size=1000):
    # 1. 데이터 로드
...
def objective(trial):
    ...
    unit_embedding_dim = trial.suggest_int('unit_embedding_dim', 4, 64)
    hash_size = trial.suggest_int('hash_size', 500, 2000)

    # 사용되는 하이퍼파라미터들을 출력
    print(f"Starting trial with parameters: lr={lr}, dropout_rate={dropout_rate}, "
          f"batch_size={batch_size}, unit_embedding_dim={unit_embedding_dim}, hash_size={hash_size}")


    num_epochs = 30

    # TensorBoard 로그 작성기(SummaryWriter) 초기화
    writer = SummaryWriter(log_dir=f'runs/travel_time/trial_{trial.number}')
    ...
    # 모델 정의
    model = TravelTimeModel(input_size, hash_size, unit_embedding_dim).to(device)
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, device, num_epochs=10, hyperparams=None):
    ...
    # TensorBoard 로그 작성기(SummaryWriter) 초기화
    hyperparam_str = '_'.join([f"{k}={v}" for k, v in hyperparams.items()]) if hyperparams else ""
    log_dir = f'runs/travel_time/{start_time}_{hyperparam_str}'
...
if __name__ == "__main__":
    ...
    # 결과 저장 폴더 생성 (사용된 하이퍼파라미터 문자열 생성)
    hyperparam_str = '_'.join([f"{k}={v}" for k, v in best_params.items()])
    results_folder = f'results_travel_time_{start_time}_{hyperparam_str}'
    ...
    # 학습된 모델 저장
    model_name = os.path.join(results_folder, f'travel_time_model_{start_time}_{hyperparam_str}.pth') # 하이퍼
    ...
    def plot_losses(train_losses, val_losses):
        ...
        plot_name = os.path.join(results_folder, f'loss_plot_travel_{start_time}_{hyperparam_str}.png')
        plt.savefig(plot_name)
        plt.close()  # 그래프 저장 후 닫기
        print(f"Plot saved as {plot_name}")
