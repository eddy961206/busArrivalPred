...
def load_data(train_csv, val_csv, test_csv, batch_size=32):
    ...
    return train_loader, val_loader, test_loader, label_encoder_route, label_encoder_location
...
def evaluate_predictions(test_loader, model, device, num_samples=10):
    ...
    # 샘플 시각화
    plot_name = os.path.join(results_folder, f'holding_time_comparison_test_{start_time}.png')
...
if __name__ == "__main__":
    ...
    # 데이터 로드 및 전처리
    train_loader, val_loader, test_loader, label_encoder_route, label_encoder_location = load_data(
    ...
    # 학습된 모델 저장
    model_name = os.path.join(results_folder, f'holding_time_model_{start_time}.pth')
    ...
    def plot_losses(train_losses, val_losses):
        ...
        plot_name = os.path.join(results_folder, f'loss_plot_holding_{start_time}.png')
        plt.savefig(plot_name)
        plt.close()  # 그래프 저장 후 닫기
        print(f"Plot saved as {plot_name}")
