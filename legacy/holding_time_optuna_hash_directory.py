...
class HoldingTimeModel(nn.Module):
    def __init__(self, input_size, hash_size, route_embedding_dim, location_embedding_dim, dropout_rate):
...
def objective(trial):
    ...
    hash_size = trial.suggest_int('hash_size', 500, 2000)   # 노선ID 877, 정류장ID 8638개
    ...
    print(f"\nStarting Trial with parameters: {trial_params}")
    ...
    # 모델 정의
    model = HoldingTimeModel(input_size, hash_size, route_embedding_dim,
...
def short_params_string(params):
    key_params = ['lr', 'dr', 'bs', 'rd', 'ld', 'hs']  # learning rate, dropout rate, batch size, route dim, location dim, hash size
