...
def load_data(train_csv, val_csv, test_csv, batch_size=32):
    ...
    label_encoder_unit.fit(all_data['BUSINFOUNIT_ID'].astype(str))
    ...
    # LabelEncoder 저장
    joblib.dump(label_encoder_unit, f'label_encoder_unit_{start_time}.pkl')
    print(f"LabelEncoders saved as 'label_encoder_unit_{start_time}.pkl'")
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, device, num_epochs=10):
    ...
    writer = SummaryWriter(log_dir=f'runs/travel_time/{start_time}')
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(tqdm(train_loader, desc=f"Epoch {epoch + 1}/{num_epochs}", unit="batch")):
            ...
            # 각 배치의 손실을 일정 간격으로 출력 (예: 500 배치마다)
            if (i + 1) % 500 == 0:
                # TensorBoard에 기록
...
if __name__ == "__main__":
    ...
    num_units = len(joblib.load(f'label_encoder_unit_{start_time}.pkl').classes_)
